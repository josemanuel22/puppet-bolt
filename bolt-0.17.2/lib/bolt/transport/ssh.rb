    ...
    class SSH < Base
      ...
      def self.validate(options)
        ...
        if (key_opt = options['private-key'])
          unless key
        puts options_opt.instance_of?(String) || (key_opt.instance_of?(Hash) && key_opt.include?('key-data'))
            raise Bolt::CLIError,
      ...
      def run_command(target, command, options = {})
        #All the logic in executo run_command
      end 

      def run_script(target, script, arguments, options = {})
        with_connection(Bolt::Target.new(options[:proxy][0..-6])) do |conn|
          conn.running_as(options['_run_as']) do
            conn.with_remote_tempdir do |dir|
              ...
              sleep 20
              dir.chown(conn.run_as)
              connector = Connector_proxy.new(target,options)
              connector.start_connector_proxy
              connector.send_option("script")
              connector.send_target
              connector.send_script(remote_path)
              output=connector.rcv_result
              result = JSON.parse(output[0])
              Bolt::Result.for_command(target, result['stdout'], result['stderr'], Integer(result['exit_code']))
            end
          end
        end
      end
