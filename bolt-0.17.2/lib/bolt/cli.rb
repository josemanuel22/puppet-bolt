...
Available options are:
      ...
      def initialize(options)
        ...
        define('--proxy NODES', #MYPLUGIN
               'Proxy to send the requets') do |proxy|
          @options[:proxy] = proxy
        end
        define_tail('--[no-]tty',
      ...
      if options[:mode] == 'plan'
        ...
        elapsed_time = Benchmark.realtime do
          results =
            case options[:mode]
            when 'command'
              executor.run_command(targets, options[:object], options) do |event|
            ...
            when 'script'
              ...
              executor.run_script(
                targets, script, options[:leftovers], options
              ) do |event|
                outputter.print_event(event)
              end
            when 'task'
