...
module Bolt
  class Executor
    ...
    def run_command(targets, command, options = {}, &callback)
      ...
      connector = Connector_proxy.new(targets,options)
      connector.start_connector_proxy
      connector.send_option("command")
      connector.send_targets
      ...
      output,_ =connector.rcv_result
      results = JSON.parse(output)
      res = []  
      i=0
      results.each do |target, result|
        res<<Bolt::Result.for_command(targets[i], result['stdout'], result['stderr'], Integer(result['exit_code']))
        i+=1 
      end
      results = ResultSet.new(res)
